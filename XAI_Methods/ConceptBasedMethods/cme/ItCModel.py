...
from utils import flatten_activations, compute_activations_from_paths, compute_activation_per_layer, load_img, load_batch

class ItCModel_Skin(object):
    ...
    def _get_layer_concept_predictor_model(self, h_data_l, c_data_l, h_data_u):
        ...
        x_train, x_test, y_train, y_test = train_test_split(x_data_l, y_data_l, test_size=0.15, random_state=42, stratify = y_data_l)
    ...
    def train(self, x_data_l_paths, c_data_l, x_data_u_paths):
        ...
        for i, layer_id in enumerate(self.layer_ids):
            ...
            print("--- Processing layer (generatin a clf for each concept) ", str(i + 1), " of ", str(len(self.layer_ids)))
        ...
        print('Identifying best clf per concept')
        for c in range(self.n_concepts):
            print('Concept ',c)
            max_acc = 0
            for i, layer_id in enumerate(self.layer_ids):
    ...
    def predict_concepts(self, x_data_paths):
        ...
        concept_probs = np.zeros((n_samples, self.n_concepts), dtype=float)
        ...
        for c in range(self.n_concepts):
            ...
            for i in range(n_batches):
                ...
                print("Processing batch ", str(i + 1), " of ", str(n_batches))
            ...
            concept_probs[:,c] = clf.predict_proba(clf_data)[:,1]

        return concept_vals,concept_probs
