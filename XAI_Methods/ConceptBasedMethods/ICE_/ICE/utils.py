...
SIZE = [224,224]#[299,299]
EPSILON = 1e-8

class img_utils():

    def __init__(self,
                img_size = (299,299),#(224,224),#(299,299),
                nchannels = 3,
    ...
    def resize_img(self,size, array, smooth = False):
        fsize = self.fsize
        size = array.shape
        if smooth:            
            tsize = list(size)
    ...
    def resize_img2(self,d_size, array, smooth = False):
        fsize = d_size
        size = array.shape
        if smooth:            
            tsize = list(size)
    ...
    def img_filter(self,x,h,size_hr,threshold=0.5,background = 0.2,smooth = True, minmax = False):
        x = x.copy()
        h = h.copy()
        if minmax:
            h = h - h.min()
        ...
        h = self.resize_img2(size_hr,h,smooth = smooth)
    ...
    def contour_img(self,x,h,dpi = 300):
        dpi = float(dpi)
        size = x.shape
        if x.max()>1:
            x = x/x.max()
        fig = plt.figure(figsize=(1.5*size[1]/dpi,1.5*size[0]/dpi),dpi=dpi)
        #fig = plt.figure(figsize=(size[1]/dpi,size[0]/dpi),dpi=dpi*SIZE[0]/size[0]) #bef
