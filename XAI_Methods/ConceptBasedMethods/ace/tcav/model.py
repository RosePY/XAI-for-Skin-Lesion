...
class PublicImageModelWrapper(ImageModelWrapper):
  ...
  def create_input(t_input, image_value_range):
    ...
    #   t_prep_input[:,:,channel] = (t_prep_input[:,:,channel] - mean[channel]) / std[channel]
    mean = [0.485, 0.456, 0.406]
    std =  [0.229, 0.224, 0.225]
    #mean2 = np.array([np.ones((299,299))* mean[0],np.ones((299,299))* mean[1],np.ones((299,299))* mean[2]]) #InceptionV4
    #std2 = np.array([np.ones((299,299))* std[0],np.ones((299,299))* std[1],np.ones((299,299))* std[2]]) 
    mean2 = np.array([np.ones((224,224))* mean[0],np.ones((224,224))* mean[1],np.ones((224,224))* mean[2]]) #ResNet50
    std2 = np.array([np.ones((224,224))* std[0],np.ones((224,224))* std[1],np.ones((224,224))* std[2]])
    
    mean2 = np.transpose(mean2,[1,2,0])
    std2 = np.transpose(std2,[1,2,0])
    mean2 = np.expand_dims(mean2,0)
    std2 = np.expand_dims(std2,0)

    t_prep_input = (t_prep_input - mean2)/std2
    #print(t_prep_input)
    #print('------------------------------------------------------------------------')
  ...
  def get_bottleneck_tensors(scope):
    ...
    for op in graph.get_operations():
      ...
      # ResNet50
      if 'AddV2' in op.type or 'Relu' in op.type:
        name = op.name.split('/')[2]
        bn_endpoints[name] = op.outputs[0]
      ##


      # #uncomment for InceptionV4
      # if 'Concat' in op.type:
      #   name = op.name.split('/')[2]
      # #if op.name.startswith(scope + '/') and 'Concat' in op.type:
      # #  name = op.name.split('/')[1]
      #   bn_endpoints[name] = op.outputs[0]
...
class InceptionV4Wrapper_public(PublicImageModelWrapper):
  def __init__(self, sess, model_saved_path, labels_path):
    ...
    self.sess = sess
    super(InceptionV4Wrapper_public, self).__init__(
        sess,
        model_saved_path,
        labels_path,
        image_shape_v4,
        endpoints_v4,
        scope='model')
    self.model_name = 'InceptionV4_public'

class ResNet50Wrapper_public(PublicImageModelWrapper):
  def __init__(self, sess, model_saved_path, labels_path):
    self.image_value_range = (0, 1)
    image_shape_v4 = [ 224, 224, 3]
    endpoints_v4 = dict(
        input='x:0',
        #logit='softmax/logits:0',
        prediction='Identity:0',
