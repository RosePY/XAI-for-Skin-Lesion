...
def make_model(sess, model_to_run, model_path,
    ...
    if model_to_run == 'InceptionV3':
        ...
        mymodel = model.GoogleNetWrapper_public(
            sess, model_saved_path=model_path, labels_path=labels_path)
    elif model_to_run == 'InceptionV4':
        mymodel = model.InceptionV4Wrapper_public( 
            sess, model_saved_path=model_path, labels_path=labels_path)
...
def plot_concepts(cd, bn,scores, num=10, address=None, mode='diverse', concepts=None):
    ...
    for image_name in sorted_files:
        image_path = masks_folder+image_name[:-4]+'_attributes.jpg'
        image = read_image(image_path,3)
        original_images_attributes.append(image)
    #print('Nombres ordenados',original_images_attributes)
    ...
    plt.rcParams['figure.figsize'] = num * 2.05, 6.05 * num_concepts
    plt.rcParams['figure.dpi'] = 200
    fig = plt.figure(figsize=(num * 2, 6 * num_concepts))
    outer = gridspec.GridSpec(num_concepts, 1, wspace=0., hspace=0.1)
    for n, concept in enumerate(concepts):
        inner = gridspec.GridSpecFromSubplotSpec(
            3, num, subplot_spec=outer[n], wspace=0.01, hspace=0.1)
        ...
        valid = ''
        if pvalue > 0.05:
            valid = ' ** Not passed statistical test'
        for i, idx in enumerate(idxs):
            #common_data
            mask = 1 - (np.mean(concept_patches[idx] == float(
                cd.average_image_value) / 255, -1) == 1)
            #print("size mask", mask.shape)
            image = original_images[concept_image_numbers[idx]]
            #print("size image", image.shape)
            height,width = image.shape[0], image.shape[1]
            mask_imaged =  Image.fromarray(mask.astype(bool))
            image_imaged = Image.fromarray(image)
            image_resized = np.array(image_imaged.resize((512,384), Image.BICUBIC))
            #print("size mask_res", mask_resized.shape)
            mask_real = np.array(mask_imaged.resize((width,height), Image.BICUBIC))
            mask_resized = np.array(mask_imaged.resize((512,384), Image.BICUBIC))
            #print("size mask_res", mask_resized.shape)
            
            #first row
            ax = plt.Subplot(fig, inner[i])
            ones = np.where(mask_real == 1)
            h1, h2, w1, w2 = ones[0].min(), ones[0].max(), ones[1].min(), ones[1].max()
            patch = np.copy(image)
            for c in range(image.shape[2]):
                patch[:, :, c] = np.where(mask_real==1, patch[:, :, c], 117)

            ax.imshow(patch[h1:h2, w1:w2])
            #ax.imshow(image_patch)
            ax.set_xticks([])
            ax.set_yticks([])
            if i == int(num / 2) - 1:
                ax.set_title(concept + ':'+ valid + ' {} +- {}, {}'.format(
            ...
            ax.imshow(mark_boundaries(image_resized, mask_resized, color=(1, 1, 0), mode='thick'))
            ax.set_xticks([])
            ax.set_yticks([])
            ...
            fig.add_subplot(ax)
            #Third row
            ax = plt.Subplot(fig, inner[i+2*num])
            ax.imshow(original_images_attributes[concept_image_numbers[idx]])
            ax.set_xticks([])
            ax.set_yticks([])
            ax.grid(False)
            fig.add_subplot(ax)
    ...
    plt.suptitle(bn)
    fig.savefig(address + bn + '.pdf', dpi=250,  bbox_inches = 'tight', pad_inches = 0)
    #if address is not None:
    #    with tf.io.gfile.GFile(address + bn + '.png', 'w') as f:
    #        fig.savefig(f,dpi = 200, bbox_inches = 'tight', pad_inches = 0)
    #    plt.clf()
    #    plt.close(fig)
...
def similarity(cd, num_random_exp=None, num_workers=25):
    ...
    randoms = ['random_500__{}'.format(i) for i in np.arange(num_random_exp)]
    concepts = {}
...
def save_ace_report(cd, accs, scores, address):
    ...
    for bn in cd.bottlenecks:
        ...
        for concept in cd.dic[bn]['concepts']:
            report += '\n' + bn + ':' + concept + ':' + str(
                np.mean(accs[bn][concept]))+ ' +- ' + str(np.std(accs[bn][concept]))
    ...
    report += '\n\t\t\t ---Random TCAV scores data---'
    for bn in cd.bottlenecks:
        report += '\n'
        report += '\n{}:{}:{} +- {}'.format(bn,cd.random_concept,
                                             np.mean(scores[bn][cd.random_concept]), np.std(scores[bn][cd.random_concept]))
        report += '\nRaw Data\n'
        report += '\n{}:{}:{}'.format(bn, cd.random_concept, scores[bn][cd.random_concept])
...
def v_predict(path_image,model):
    a = load_image_from_file(path_image, (224,224))
    print (a.shape)
    images = np.array([a])
    results = model.get_predictions(images)
    print(results)
...
def plot_concepts_original(cd, bn,scores, num=10, address=None, mode='diverse', concepts=None): #before it says diverse on mode
    """Plots examples of discovered concepts.
    Args:
      cd: The concept discovery instance
      bn: Bottleneck layer name
    ...
    plt.suptitle(bn)
    if address is not None:
        with tf.io.gfile.GFile(address + bn + '_small.png', 'w') as f:
            fig.savefig(f)
        plt.clf()
        plt.close(fig)
